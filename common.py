...
def annotation_recorder(
    ...
    for i in index:
        ...
        manual_annotation = np.append(
            manual_annotation,
            m
        )

    if len(index) % 11 == 0:
        manual_annotation = manual_annotation.reshape(
            len(index) // 11,
            11
            )
...
def get_beat_slices(
    ...
    if is_avg:
        for _ in moving_average_ranges:
            beat_slice_shape = (beat_slice_shape[0] + 1, beat_slice_shape[1])
    if is_derivative:
        beat_slice_shape = (beat_slice_shape[0] + 1, beat_slice_shape[1])
    if is_points:
    ...
    if beat_slice_shape[0] == 0:
        raise ValueError(
            f'get_beat_slices requires at least 1 data source: raw signal, moving average, '
            f'derivative or manual '
            f'label 11 points; but none were selected {manual_label_dict=} {moving_average_ranges=} '
    ...
    for row in beats.itertuples():
        ...
        if manual_label_dict:
            ...
            manual_label_dict_sample_key = manual_r_peaks[np.where(
        ...
        if is_raw or is_avg or is_derivative:
            if row.sample - window_size < 0:
                ...
                beat_slice[last_empty_channel, :-abs(row.sample + window_size - signal.size)] = signal[
                                                                                                row.sample - window_size:]
            else:
        ...
        if is_avg:
            for mar in moving_average_ranges:
                beat_slice[last_empty_channel, :] = np.convolve(
                    beat_slice[last_empty_channel, :],
                    np.ones(mar),
                    'same'
                ) / mar
                last_empty_channel += 1
        ...
        #     if is_avg:
        #         beat_slice[0, :] = np.convolve(beat_slice[0, :], np.ones(moving_average_ranges),
        #                                        'same') / moving_average_ranges
        ...
        #     else:
        #         beat_slice[0, :] = signal[row.sample - window_size: row.sample + window_size]
        #     beat_slice[1, :] = np.convolve(beat_slice[1, :], np.ones(moving_average_ranges),
        #                                    'same') / moving_average_ranges
        ...
        #     if is_avg:
        #         beat_slice[:] = np.convolve(beat_slice[:], np.ones(moving_average_ranges), 'same') /
        #         moving_average_ranges
        # elif not is_points and is_avg and is_raw:
        #     # Moving average + raw signal
        #     if row.sample - window_size < 0:
        #         beat_slice[0, abs(row.sample - window_size):] = signal[0: row.sample + window_size]
