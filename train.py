...
def run(
    moving_average_ranges, use_classes_from_manual_labels, subset_from_manual_labels, include_manual_labels,
    include_raw_signal, include_derivative
):

    MOVING_AVERAGE_RANGES = moving_average_ranges
    ...
    CLASSES = ['N', 'L', 'R', 'a', 'V', 'J', 'F'] if USE_CLASSES_FROM_MANUAL_LABELS else ['N', 'L', 'R', 'A', 'a', 'V',
                                                                                          'j', 'J', 'E', 'f', 'F', '[',
                                                                                          '!', ']', '/', 'x', '|', 'Q']
    ...
    if MOVING_AVERAGE_RANGES:
        for mar in MOVING_AVERAGE_RANGES:
            if RUN_NAME:
                RUN_NAME += '_and_'
            RUN_NAME += f'moving_average-{mar}'
    ...
    dataset = ArrhythmiaDataset(
        RECORD_DIR_PATH, WINDOW_SIZE, only_include_labels = CLASSES, moving_average_ranges =
        MOVING_AVERAGE_RANGES, include_manual_labels = INCLUDE_MANUAL_LABELS, subset_from_manual_labels =
        SUBSET_FROM_MANUAL_LABELS, include_raw_signal =
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        ...
        patience = 7,
        verbose = True
    )  # reduce learning after 7 epochs with no improvement
    ...
    writer.add_hparams(
        {f'data_shape_{i}': shape for i, shape in enumerate(dataset.data.shape)} | {
            **{
                f'data_moving_average_range_{idx}': mar for idx, mar in enumerate(MOVING_AVERAGE_RANGES)
            },
        ...
        {
            'hparam/fake_accuracy_just_to_have_any_metric': 10
        },
        run_name = RUN_NAME
        )
    ...
    with tqdm(total = n_epoch) as pbar:
        for epoch in range(1, n_epoch + 1):
            ...
            if len(accuracies) >= ACCURACY_MOVING_AVERAGE_SIZE + 1:
                is_performance_degraded = np.mean(accuracies[-ACCURACY_MOVING_AVERAGE_SIZE - 1:-1]) > np.mean(
                    accuracies[-ACCURACY_MOVING_AVERAGE_SIZE:]
                )
